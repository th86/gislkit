...
void caf(const double *data, const double *vec, double *mi, double a, int maxIter, double epsilon, int m, int n, int bin, int so, int negateMI, int verbose, int strengthPos, double strengthThreshold){
	...
	if(oi[strengthPos]>strengthThreshold){
...
SEXP cafquickR2C(SEXP dataIn, SEXP vecIn, SEXP aIn, SEXP maxIterIn, SEXP epsilonIn, SEXP mIn, SEXP nIn, SEXP binIn, SEXP soIn, SEXP negateMIIn, SEXP verboseIn, SEXP strengthPosIn, SEXP strengthThresholdIn){
	SEXP miOut;
	double *data, *vec, *mi, a, epsilon, strengthThreshold;
	int maxIter, m, n, bin, so, negateMI, verbose, strengthPos;
	R_len_t mo;
	...
	PROTECT(strengthPosIn = AS_INTEGER(strengthPosIn));
	PROTECT(strengthThresholdIn = AS_NUMERIC(strengthThresholdIn));
	...
	strengthPos = INTEGER_POINTER(strengthPosIn)[0];
	strengthThreshold = NUMERIC_POINTER(strengthThresholdIn)[0];	
	...
	caf(data, vec, mi, a, maxIter, epsilon, m, n, bin, so, negateMI, verbose, strengthPos, strengthThreshold);
